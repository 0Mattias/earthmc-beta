...
export async function GET(request: NextRequest) {
    try {
        ...
        let whereClause = `snapshot_ts = (
            SELECT COALESCE(
                (SELECT MAX(snapshot_ts) FROM nation_snapshots WHERE snapshot_ts < (SELECT MAX(snapshot_ts) FROM nation_snapshots)),
                (SELECT MAX(snapshot_ts) FROM nation_snapshots)
            )
        )`;
