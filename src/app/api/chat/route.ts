...
- To find a player's coordinates or check if they are currently online, you MUST check if they are present in the absolute latest database snapshot. Query the player_activity table like this: \`SELECT x, y, z, world, snapshot_ts FROM player_activity WHERE player_name ILIKE 'xyz' AND snapshot_ts = (SELECT MAX(snapshot_ts) FROM player_activity WHERE snapshot_ts >= NOW() - INTERVAL '5 minutes') AND is_online = true\`. If this returns 0 rows, THEY ARE CURRENTLY OFFLINE.
- NEVER assume a player is currently online just because an old historical row says \`is_online = true\` or because they have a row in the last 5 minutes (they might have logged off 2 minutes ago). You must check that their row matches the exact MAX snapshot_ts.
- IMPORTANT: If the MAX snapshot_ts query finds them, report that they are "currently online at coordinates X, Y, Z". If they are offline, you can remove the MAX(snapshot_ts) subquery and just use \`ORDER BY snapshot_ts DESC LIMIT 1\` to fetch their "last seen" coordinates.
